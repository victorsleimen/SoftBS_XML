...
public class UtilImages {
	...
	public static File limit(File image, int size) throws IOException {
		...
		if (limitedImage == null || limitedImage.getFile().lastModified() < image.lastModified()
			...
			if (source.width > size || source.height > size) {
				...
				limitedImageCache.put(image, new ScaledImage(scale(image, target), size));
			} else {
				limitedImageCache.put(image, new ScaledImage(scale(image, source), size));
			}
		}
	...
	/**
	 * Scales given image file with given size (in pixels) and caches the result.
	 */
	public static File scale(File image, int size) throws IOException {
		ScaledImage scaledImage = scaledImageCache.get(image);
		if (scaledImage == null || scaledImage.getFile().lastModified() < image.lastModified()
				|| scaledImage.getSize() != size) {
			Dimension source = getSize(image);
			Dimension target = scaleDimension(source, new Dimension(size, size));
			scaledImageCache.put(image, new ScaledImage(scale(image, target), size));
		}
	 ...
	 */
	public static File scale(File image, Dimension targetDimension) throws IOException {
		OutputStream output = null;
		try {
			File targetFile = File.createTempFile("img-", ".tmp");
			output = new FileOutputStream(targetFile);
			ImageIO.write(getScaledInstance(ImageIO.read(image), (int) targetDimension.getWidth(),
					(int) targetDimension.getHeight()), getFormat(image), output);
			return targetFile;
		} finally {
			if (output != null) {
				output.close();
	...
	public static File rotate(File image) throws IOException {
		...
		try {
			File targetFile = File.createTempFile("img-", ".tmp");
			output = new FileOutputStream(targetFile);
			ImageIO.write(getRotatedInstance(ImageIO.read(image)), getFormat(image), output);
			return targetFile;
		} finally {
			if (output != null) {
				output.close();
	...
	public static String getFormat(File image) throws IOException {
		ImageInputStream stream = ImageIO.createImageInputStream(new FileInputStream(image));
	...
	public static boolean isBrowserCompatible(String contentType) {
		return "image/jpeg".equals(contentType) || "image/gif".equals(contentType) || "image/png".equals(contentType);
	}
}
