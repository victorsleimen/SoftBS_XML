...
public final class UtilFile {
	...
	final static Logger logger = LoggerFactory.getLogger(UtilFile.class);
	public static final String module = UtilFile.class.getName();
	...
	private static class SearchTextFilesFilter implements FilenameFilter {
		...
		public SearchTextFilesFilter(String fileExtension, Set<String> stringsToFindInPath,
				Set<String> stringsToFindInFile) {
		...
		public boolean accept(File dir, String name) {
			...
			if (hasAllPathStrings && name.endsWith("." + fileExtension)) {
				...
				try {
					xmlFileBuffer = UtilFile.readTextFile(file, true);
				} catch (FileNotFoundException e) {
					logger.debug("Error reading xml file [" + file + "] for file search: " + e.toString(), module);
					return false;
				} catch (IOException e) {
					logger.debug("Error reading xml file [" + file + "] for file search: " + e.toString(), module);
					return false;
	...
	public static void searchFiles(List<File> fileList, File path, FilenameFilter filter, boolean includeSubfolders)
			throws IOException {
	...
	public static List<File> findFiles(String fileExt, String basePath, String partialPath, String stringToFind)
			throws IOException {
		if (basePath == null) {
		...
		UtilFile.searchFiles(fileList, new File(basePath),
				new SearchTextFilesFilter(fileExt, stringsToFindInPath, stringsToFindInFile), true);

		return fileList;
	}

	public static List<File> findXmlFiles(String basePath, String partialPath, String rootElementName,
			String xsdOrDtdName) throws IOException {
		if (basePath == null) {
			basePath = System.getProperty("ofbiz.home");
		}
		...
		List<File> fileList = new LinkedList<File>();
		UtilFile.searchFiles(fileList, new File(basePath),
				new SearchTextFilesFilter("xml", stringsToFindInPath, stringsToFindInFile), true);
		return fileList;
	}
	 ...
	 *
	 * @param reader
	 *            A Reader in which the String will be searched.
	 * @param searchString
	 ...
	 *
	 *
	 * Search for the specified <code>searchString</code> in the given filename. If
	 * the specified file doesn't exist, <code>FALSE</code> returns.
	 *
	 * @param fileName
	 *            A full path to a file in which the String will be searched.
	 * @param searchString
	 *            The String to search for
	 * @return <code>TRUE</code> if the <code>searchString</code> is found;
	 *         <code>FALSE</code> otherwise.
	 ...
	 *
	 *
	 * Check if the specified <code>fileName</code> exists and is a file (not a
	 * directory) If the specified file doesn't exist or is a directory
	 * <code>FALSE</code> returns.
	 *
	 * @param fileName
	 *            A full path to a file in which the String will be searched.
	 * @return <code>TRUE</code> if the <code>fileName</code> exists and is a file
	 *         (not a directory) <code>FALSE</code> otherwise.
	 */
