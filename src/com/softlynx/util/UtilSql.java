...
public class UtilSql {
	private static Logger logger = Logger.getLogger(UtilSql.class.getName());
	 ...
	 * 
	 * @return Always a null value so the statement may be nulled out and closed in
	 *         one line.
	 * 
	 */
	 ...
	 * 
	 * @return Always a null value so the connection may be nulled out and closed in
	 *         one line.
	 * 
	 */
	 ...
	 * 
	 * @return Always a null value so the ResultSet may be nulled out and closed in
	 *         one line.
	 * 
	 */
	...
	public static List<Map<String, Object>> getResultSet(ResultSet rs) throws SQLException {
		...
		if (rs != null) {
			...
			for (int r = 1; rs.next(); r++) {
				...
				for (i = 1; i <= numCols; i++) {
					logger.fine(" COLUMN: '" + rsmd.getColumnName(i) + "'='" + rsmd.getColumnTypeName(i) + "'");
					if (rsmd.getColumnTypeName(i).equals("DATE")) {
						row.put(rsmd.getColumnLabel(i).trim(), rs.getTimestamp(i));
					} else {
						row.put(rsmd.getColumnLabel(i).trim(), "" + rs.getString(rsmd.getColumnLabel(i)));
					}
				}
	...
	public static List<Map<String, Object>> getResultSetNew(ResultSet rs) throws SQLException {
		int i = 0;
		Map<String, Object> row = null;
		List<Map<String, Object>> resultList = new ArrayList<Map<String, Object>>();
			...
			while (rs.next()) {
				...
				for (i = 1; i <= numCols; i++) {
					if (rsmd.getColumnTypeName(i).equals("DATE")) {
						...
						row.put(rsmd.getColumnLabel(i), "" + rs.getString(rsmd.getColumnLabel(i)));
					}
				}
	...
	public static Connection getConnection(DataSource ds, String username, String password, int numTries,
		...
		for (int i = 0; i < numTries; i++) {
			try {
				...
				if (i < (numTries - 1)) {
					logger.severe("Attempt #" + (i + 1)
							+ ", Unable to create connection from DataSource, trying again in " + retryWait + " ms.");
					try {
	...
	public static Connection getConnection(DataSource ds, int numTries, long retryWait) throws SQLException {
		Connection con = null;
