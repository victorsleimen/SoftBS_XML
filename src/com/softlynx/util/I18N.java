...
public class I18N {
	...
	static private class MessageBundle extends ResourceBundle {
		...
		public Enumeration<String> getKeys() {
			...
			return new ResourceBundleEnumeration(messages.keySet(), (parent != null) ? parent.getKeys() : null);
		}
		...
		private List<String> getBundleNames(String baseName) {
			...
			try {
				...
				Enumeration<URL> names = Thread.currentThread().getContextClassLoader().getResources(resourceName);
				while (names.hasMoreElements()) {
					URL jarUrl = names.nextElement();
					String jarFile = URLDecoder.decode(jarUrl.getFile(), "UTF-8");
					if ("jar".equals(jarUrl.getProtocol())) {
						String filename = jarFile.substring(0, jarFile.length() - resourceName.length() - 2);
						...
						for (Enumeration<JarEntry> entries = jar.entries(); entries.hasMoreElements();) {
							...
							addMatch("", baseName, bundleNames, baseFileName, name);
						}
						jar.close();
					} else {
						...
						if (jarDirectory.isDirectory()) {
							for (String name : jarDirectory.list()) {
								addMatch(resourcePath, baseName, bundleNames, baseFileName, name);
							}
						}
					}
				}
		...
		private void addMatch(String resourcePath, String baseName, List<String> bundleNames, String baseFileName,
				String name) {
		...
		static private class ResourceBundleEnumeration implements Enumeration<String> {
